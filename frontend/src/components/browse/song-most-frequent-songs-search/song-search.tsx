...
export class SongSearch extends React.Component<IProps, ISongSearchState> {
    ...
    public add = () => {
        ...
        if (selectedSong.spotifyTrackId && !this.props.songsForMostFrequentSongsSearch.some((song: Song) => song.spotifyTrackId === selectedSong.spotifyTrackId)) {
            this.props.addSongForMostFrequentSongsSearch(selectedSong);
            this.setState({
                selectedSong: new Song(),
                songValue: ''
            });
